...
class BidderMicroservice {
    ...
    private var name: String = "[BIDDER_NECONECTAT]"
    private var phoneNumber: String = "[BIDDER_NECONECTAT]"
    ...
    companion object Constants {
        ...
        private val firstNames = listOf("Alice", "Bob", "Charlie", "David", "Emma", "Frank", "Grace", "Henry", "Isabella", "Jack",
            "Kate", "Liam", "Mia", "Noah", "Olivia", "Peter", "Quinn", "Rachel", "Sophia", "Thomas",
        ...
        fun generatePhoneNumber():String {
            var aux = ""
            for (i in 1..10) {
                aux += Random.nextInt(0, 9)
            }
            return aux
        }
        fun generateEmail(name:String):String {
            return name.toLowerCase().replace(" ", "") + "@" + emailDomains.random()
        }
    }
    ...
    private fun bid() {
        ...
        val biddingMessage = Message.create("${auctioneerSocket.localAddress}:${auctioneerSocket.localPort}",
            "licitez $pret", name, phoneNumber, email)
