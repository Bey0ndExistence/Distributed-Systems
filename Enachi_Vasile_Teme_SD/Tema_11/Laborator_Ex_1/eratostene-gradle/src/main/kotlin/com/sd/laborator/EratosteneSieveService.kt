...
class EratosteneSieveService {
    ...
    */
    ...
    fun findPrimesLessThanRecursive(n: Int): List<Int> {
        val prime: MutableList<Int> = ArrayList()
        findPrimesLessThanRecursiveCall(2, n, prime)
        return prime
    }


    private fun findPrimesLessThanRecursiveCall(currentNumber: Int, n: Int, prime: MutableList<Int>) {
        if (currentNumber < n){
            if (isPrime[currentNumber]) {
                prime.add(currentNumber)
                SPF[currentNumber] = currentNumber
            }
            var j = 0
            while (j < prime.size && currentNumber * prime[j] < n && prime[j] <= SPF[currentNumber]) {
                isPrime[currentNumber * prime[j]] = false
                SPF[currentNumber * prime[j]] = prime[j]
                j++
            }
            findPrimesLessThanRecursiveCall(currentNumber+1, n, prime)
        }
    }
