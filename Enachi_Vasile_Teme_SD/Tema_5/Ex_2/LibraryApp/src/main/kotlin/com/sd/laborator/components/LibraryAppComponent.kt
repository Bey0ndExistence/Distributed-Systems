...
class LibraryAppComponent {
    ...
    fun recieveMessage(msg: String) {
        ...
        try {
            ...
            println("Functie : {$function}")
            if (function == "add"){
                customAddBook(parameter)
                sendMessage("Adaugarea a avut loc cu succes")
                return
            }
    ...
    fun customAddBook(format: String) {
        val (author, title,  editura, content) = format.split("=")
        addBook(Book(Content(name=title, author=author, publisher=editura,  text=content)))
    }
