...
public class ProcessStudentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if (!file.exists()) {
            ...
            student_list.setStudent_list(new ArrayList<StudentBean>());
        } else{
            student_list = mapper.readValue(file, StudentList.class);
        }
        student_list.getStudent_list().add(bean);

        mapper.writeValue(file, student_list);
