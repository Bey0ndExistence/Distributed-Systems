...
class StudentMicroservice {
    ...
    companion object Constants {
        ...
        val CONTAINER_NAME = System.getenv("CONTAINER_NAME") ?: null
    }
    ...
    private fun messageManagerLoop() {
    ...
    private fun subscribeToHeartbeat() {
        if (CONTAINER_NAME == null) {
            // silent failure, daca nu merge cu heartbeat aiae
            println("NU AM CONTAINER_NAME :(")
            return;
        }
        ...
        try {
            ...
            // exitProcess(1) nu dam exit pt ca am incalca solid (liskov)
        }
    }

    private fun heartbeatLoop() {
        if (!this::heartbeatSocket.isInitialized) {
            // silent failure, daca nu merge cu heartbeat aiae
            println("NU S A INITIALIZAT HEARTBEAT :'(")
            return;
        }

        println("Se comunica cu heartbeat pe portul: ${heartbeatSocket.localPort}")
        println("Se asteapta mesaje...")

        val bufferReader = BufferedReader(InputStreamReader(heartbeatSocket.inputStream))

        heartbeatSocket.getOutputStream().write("$CONTAINER_NAME\n".toByteArray())

        while (true) {
            // se asteapta mesajul de "esti in viata?"
            val response = bufferReader.readLine()

            if (response == null) {
                // daca se primeste un mesaj gol (NULL), atunci inseamna ca cealalta parte a socket-ului a fost inchisa
                println("Heartbeat (${heartbeatSocket.port}) a fost oprit.")
                bufferReader.close()
